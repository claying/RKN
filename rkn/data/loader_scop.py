...
def load_data(datadir, seq_list, pre_padding=0, maxlen=None, pre_value=0.0,
              return_len=True, label_file=None):
    ...
    lengths = np.asarray(
        [np.clip(len(f) + 2 * pre_padding, a_min=0, a_max=maxlen) for f in features])
    ...
    return TensorDataset(
        features, labels, lengths, noise=0.0, max_index=features.shape[-1],
